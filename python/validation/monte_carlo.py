...
class MonteCarlo:
    ...
    def run(self, trades: list[dict]) -> dict:
        ...
        """
        zero_stats = {
            "mean": 0, "median": 0, "pct_5": 0, "pct_25": 0, "pct_75": 0, "pct_95": 0,
        }

        if not trades:
            return {
                "iterations": 0,
                "final_equity": zero_stats,
                "max_drawdown": zero_stats,
                "sharpe_ratio": zero_stats,
                "profit_factor": zero_stats,
        ...
        # Arrays for per-iteration Sharpe and profit factor
        ...
        for i in range(self.iterations):
            ...
            drawdowns = (peak - equity_values) / np.where(peak > 0, peak, 1.0)
            max_drawdowns[i] = np.max(drawdowns) if len(drawdowns) > 0 else 0.0

            # Sharpe ratio (approximate from bootstrap sample)
        ...
        # Replace inf values with a large finite number for stats computation
        mc_pf_finite = np.where(np.isinf(mc_pf_values), 1e6, mc_pf_values)
        ...
        return {
            ...
            "profit_factor": self._compute_metric_stats(mc_pf_finite),
