...
class PaperTrader:
    ...
    def run_cycle(self) -> list[dict]:
        ...
        Returns:
            List of result dicts from AIEngine.run_once().
            Empty list if no symbols or all symbols failed.
        ...
        try:
            ...
            for r in successes:
                ...
                logger.info(
                    f"  {r.get('symbol', '?')}: regime={r.get('regime', '?')} "
                    f"conf={r.get('confidence', 'N/A')} "
                    f"sl={adapted.get('sl_pips', '?')}pips "
                    f"tp={adapted.get('tp_pips', '?')}pips "
                    f"lot={adapted.get('lot_size', '?')}"
                )
