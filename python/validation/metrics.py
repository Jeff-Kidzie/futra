...
def _daily_returns(equity_values: np.ndarray) -> np.ndarray:
    ...
    Returns normalized returns between consecutive equity points.
    """
...
def compute_all_metrics(
    ...
    return {
        ...
        "avg_win_loss": round(awl, 4) if awl != float('inf') else None,
        "total_return": round(compute_total_return(equity_curve), 4),
        "calmar_ratio": round(cr, 4) if cr != float('inf') else None,
